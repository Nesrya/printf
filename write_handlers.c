 ...
 * @is_negative: a Lista of arguments
 * @ind: the char types.
	...
	int length, char padd, char extra_c)
{
	...
	if (width > length)
	{
		for (i = 1; i < width - length + 1; i++)
			buffer[i] = padd;
		buffer[i] = '\0';
		if (flags & F_MINUS && padd == ' ')/* to Asign extra char to left of buffer */
		{
			if (extra_c)
				buffer[--ind] = extra_c;
...
int write_unsgnd(int is_negative, int ind,
	...
	int flags, int width, int precision, int size)
{
	/* now The number is stored at the bufer's right and starts at position i */
	...
	if (width > length)
	{
		...
		if (flags & F_MINUS) /* this Asign extra char to left of buffer [buffer>padd]*/
		{
			return (write(1, &buffer[ind], length) + write(1, &buffer[0], i));
		}
		else /* this Asign extra char to left of padding [padd>buffer]*/
		{
	...
	int width, int flags, char padd, char extra_c, int padd_start)
{
	...
	if (width > length)
	{
		for (i = 3; i < width - length + 3; i++)
			buffer[i] = padd;
		buffer[i] = '\0';
		if (flags & F_MINUS && padd == ' ')/* this Asign extra char to left of buffer */
		{
			buffer[--ind] = 'x';
			buffer[--ind] = '0';
			if (extra_c)
